...
const SideBar: React.FC<{ theme?: string; toggleTheme: () => void }> = ({ theme, toggleTheme }) => {
  return (
    <div className="absolute flex flex-col items-center top-0 left-0 w-12 h-full bg-gray-200 dark:bg-gray-900 text-black dark:text-white shadow-lg">
...
const SideBarIcon = ({
  ...
  return (
    <div className={`${styles.SidebarIcon}`} onClick={onClick}>
      {icon}
      {text && <span className={`${styles.SidebarTooltip} `}>{text}</span>}
    </div>
  )
}
